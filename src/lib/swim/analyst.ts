 ...
 *    the code-computed sum of the model's own itemised setByStepLog
 *    entries, so the reported total can never drift from (or invent beyond)
 *    what the model itemised set by set.
...

{
  "sessionTitle": string,
  "setByStepLog": [
    { "distanceM": number, "stroke": string, "equipment": string[], "interval": string | null, "zone": "REC" | "TECH" | "EN1" | "EN2" | "EN3" | "SP1" | "SP2", "notes": string | null }
  ],
  "overview": {
...
Do NOT include a top-level total-distance field -- the caller sums setByStepLog itself.

Every setByStepLog entry must correspond to something explicitly stated or unambiguously implied by the athlete's own shorthand -- never invent a rep count, distance, or interval. When a set is genuinely ambiguous, still give your single best reading (so totals stay computable) but add a matching entry to "ambiguities" naming what was uncertain and what the other genuine reading(s) would have been. An empty "ambiguities" array means nothing was ambiguous, not that you skipped checking.`;
...
function normalizeIntensityDistribution(raw: unknown, setByStepLog: AnalystSetEntry[]): Record<AnalystZone, number> {
  ...
  for (const set of setByStepLog) out[set.zone] += set.distanceM;
  return out;
}

function buildUserMessage(rawText: string, previousSessions?: AnalystPreviousSession[]): string {
  if (!previousSessions || previousSessions.length === 0) return rawText;
...
export async function analyzeSwimSession(
  ...
  const content = await callGroqChat(
    ...
    { jsonMode: true, temperature: 0.15, timeoutMs: 30_000 }
  );
  ...
  try {
    ...
    const parsedDistanceM = setByStepLog.reduce((sum, s) => sum + s.distanceM, 0);
